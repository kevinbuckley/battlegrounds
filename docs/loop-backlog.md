...
- [x] [S] Add tests/shop/metaltooth-leaper.test.ts — verify Metaltooth Leaper (tier 2 mech, 3/3) battlecry gives ALL friendly Mechs +2 ATK; board: [Mech 1/1, Mech 2/2, non-Mech 1/1]; play Metaltooth → only Mechs gain +2 ATK; golden version gives +4 ATK — tests/shop/metaltooth-leaper.test.ts

- [x] [S] Add tests/simulation/houndmaster-shaw.sim.test.ts — verify Houndmaster Shaw (tier 4 beast, 3/6) onStartOfCombat gives ALL other friendly minions the Rush keyword; board: [Shaw, vanilla 1/1, vanilla 2/2] vs [taunt 5/5]; verify both vanillas gain Rush (can attack any target), Shaw itself does NOT gain Rush; also verify a solo Shaw board gives no Rush to anyone — tests/simulation/houndmaster-shaw.sim.test.ts

- [x] [S] Add tests/simulation/fiendish-servant.sim.test.ts — verify Fiendish Servant (tier 1 demon, 2/1) onDeath gives its current ATK to a random friendly minion; board: [Fiendish Servant 2/1, vanilla 1/4] vs [3/3]; Fiendish Servant attacks and dies → friendly vanilla gains +2 ATK (3/4); also verify a buffed Servant (4 ATK) gives +4; also verify with no other friendly minions → no crash — tests/simulation/fiendish-servant.sim.test.ts

- [x] [S] Add tests/simulation/lil-exorcist.sim.test.ts — verify Lil' Exorcist (tier 3, 2/2, taunt) battlecry gives itself +1/+1 for each Deathrattle minion on both boards; build shop state with [Lil Exorcist in hand] and [Infested Wolf (deathrattle), plain 2/2] on board, enemy board has [Kaboom Bot (deathrattle)]; play Lil' Exorcist → it should gain +2/+2 (2 deathrattle minions); also verify 0 deathrattles → no buff — tests/simulation/lil-exorcist.sim.test.ts  <!-- DONE: 4 tests pass, bun typecheck + bun test 835/835 -->

- [ ] [S] Add tests/shop/screwjank-clunker.test.ts — verify Screwjank Clunker (tier 3 mech, 2/5) battlecry gives a friendly Mech +2/+2; board: [Mech 1/1, non-Mech 2/2]; play Screwjank → Mech becomes 3/3; non-Mech unchanged; no Mechs → no buff — tests/shop/screwjank-clunker.test.ts

- [ ] [S] Add tests/shop/southsea-strongarm.test.ts — verify Southsea Strongarm (tier 3 pirate, 5/3) battlecry gives a friendly Pirate +2/+2 for each Pirate bought this turn; buy 2 Pirates then Strongarm → friendly Pirate gets +4/+4; buy 0 Pirates then Strongarm → no buff — tests/shop/southsea-strongarm.test.ts

- [ ] [S] Add tests/shop/crystalweaver.test.ts — verify Crystalweaver (tier 4, 5/4) battlecry gives ALL friendly Demons +5/+5; board: [Demon 1/1, non-Demon 2/2, Demon 3/3]; play Crystalweaver → both Demons gain +5/+5; non-Demon unchanged — tests/shop/crystalweaver.test.ts

- [ ] [S] Add tests/simulation/harvest-golem.sim.test.ts — verify Harvest Golem (tier 2 beast, 2/3) deathrattle summons a 2/1 Chicken; board: [Harvest Golem] vs [3/3]; Golem dies → 2/1 Chicken appears; verify transcript has Summon event for Chicken; verify golden Harvest Golem summons a 4/2 Chicken — tests/simulation/harvest-golem.sim.test.ts

- [ ] [S] Add tests/simulation/imprisoner.sim.test.ts — verify Imprisoner (tier 2 demon, 3/3, taunt) deathrattle summons a 3/3 Imp with no keywords; board: [Imprisoner] vs [4/1 killer]; Imprisoner dies → 3/3 Imp token appears; Imp has Demon tribe; golden Imprisoner spawns 2 Imps — tests/simulation/imprisoner.sim.test.ts

- [ ] [S] Add tests/simulation/hangry-dragon.sim.test.ts — verify Hangry Dragon (tier 2 dragon, 5/4) gains +3/+3 at start of each combat round where it did NOT attack in the previous round; board: [Hangry Dragon] vs [20/20]; after the first round where Dragon waits (enemy attacks Dragon), Dragon gains +3/+3 = 8/7; verify Stat event in transcript — tests/simulation/hangry-dragon.sim.test.ts

- [ ] [S] Add tests/simulation/floating-watcher.sim.test.ts — verify Floating Watcher (tier 4 demon, 4/4) gains +2/+2 each time your hero takes non-combat damage (e.g., face damage); board: [Floating Watcher] vs [3/3 attacker]; after combat ends and hero takes damage, Floating Watcher gains +2/+2; verify via onHeroDamaged or end-of-combat Stat event — tests/simulation/floating-watcher.sim.test.ts

- [ ] [S] Add tests/shop/menagerie-magician.test.ts — verify Menagerie Magician (tier 4, 4/4) battlecry gives a friendly minion of each different tribe +2/+2; board: [Murloc 1/1, Beast 2/2, Demon 3/3, non-tribe 4/4]; play Magician → one of each Murloc, Beast, Demon gains +2/+2; non-tribe unchanged — tests/shop/menagerie-magician.test.ts
