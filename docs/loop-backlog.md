...
- [ ] [S] Add tests/shop/grimspeaker.test.ts — verify Grimspeaker (tier 3, 3/3 Demon) onBattlecry gives the first friendly Demon on board +2/+2 and adds taunt; read src/game/minions/tier3/grimspeaker.ts; board: [Vulgar Homunculus 2/4 Demon, Alley Cat 1/1 Beast]; play Grimspeaker → Homunculus becomes 4/6 and gains taunt; Beast unchanged; board with no Demons → no buff; Grimspeaker does NOT buff itself (it targets board[i] before being played) — tests/shop/grimspeaker.test.ts
...
- [ ] [S] Add tests/simulation/grombi-the-rotunda.sim.test.ts — verify Grombi the Rotunda (tier 2, 2/3 Murloc, magnetic) onAllyKill gives itself +2/+2 each time an ally kills a minion; read src/game/minions/tier2/grombi-the-rotunda.ts (onAllyKill hook, no emit); test 1: [Grombi 2/3, 5/5 ally] vs [1/1, 1/1]; ally kills first 1/1 → Grombi gains +2/+2 → 4/5; ally kills second → Grombi 6/7; Grombi survives with 6/7; test 2: [Grombi 2/3] vs [1/1]; Grombi attacks and kills → no ally kill (Grombi itself attacks, not ally) → Grombi does NOT gain stats (hook says "ally", not self) — IMPORTANT: check if the onAllyKill hook fires when self attacks and kills (it likely does NOT since self is the killer); verify by checking survivor atk — tests/simulation/grombi-the-rotunda.sim.test.ts
